...
<html>
    ...
    <body>
        ...
        $Tsavings = DB::table('mil_savings')
                        ->where('user_id', $user->id)
                        ->groupBy('user_id')
                        ->select(DB::raw('sum(savings) as savings'), 'user_id')->first();

        $totalCost = DB::table('mil_cost')->select(DB::raw('sum(cost) as cost'))->first();

        $milArr = DB::table('mil_count')->select('id', 'user_id', 'status', 'date')->get();
